...
✅ Удобное подключение: простые пошаговые инструкции для любого устройства
  ...
  ABOUT_SERVICE_PAGE: {
    text: `
🛡️ Официальная информация о VPN-сервисе GhostlineVPN

⸻

📄 Описание услуги

Мы предоставляем платную услугу — доступ к высокоскоростному VPN-серверу, расположенному в Нидерландах (Амстердам).
Сервис позволяет обойти блокировки, сохранить анонимность в сети и обеспечить стабильный доступ к нужным ресурсам.

Доступ предоставляется по безопасному протоколу (VLESS + Reality) с высокой скоростью и минимальными задержками.

⸻

💰 Тарифы
	•	Триал-доступ — 7 дней бесплатно
	•	Месячный тариф — 290 ₽
	•	Полугодовой тариф — 1390 ₽

Оплата производится в рублях РФ. Все подключения обслуживаются круглосуточно, доступ предоставляется моментально после активации.
...
📞 Контактные данные
	...
	•	Служба поддержки: ghostlinevpn@proton.me
	•	Telegram: @HomsiDark
...
export const BUTTONS: Record<string, TelegramButton> = {
  ...
  ABOUT_SERVICE: {
    text: 'ℹ️ О сервисе и ценах', // 12 символов
